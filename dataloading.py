...
'''
    ...
    sampler_type 参数表示采样器的类型，可以是 "topk" 或 "uniform"。
    如果是 "topk"，则选择最近的 k 个节点作为邻居。
    如果是 "uniform"，则随机选择 k 个节点作为邻居。
    k 参数表示每个节点最多采样的邻居数量，默认为 10。
    
    2.sampler_frontier 方法：
    
    此方法用于根据给定的种子节点和时间戳生成采样子图。
    首先，构建包含所有邻居节点的子图。
    然后，根据时间戳筛选出边，仅保留在时间戳之前发生的边。
    最后，根据选择的采样器类型从子图中选择邻居节点。
    返回最终的子图，其中包含了根据时间戳筛选和采样后的邻居节点。
    
    3.sample_blocks 方法：
    
    此方法用于对给定的种子节点和时间戳进行采样，并生成采样块。
    首先，使用 sampler_frontier 方法生成采样子图。
    然后，将采样子图转换为块表示。
    最后，返回包含采样块的列表。
...
'''
    ...
    接受一系列参数，包括输入图 g、边集 eids、邻居采样器 graph_sampler 等。
    
    2._collate_with_negative_sampling 方法：
    
    此方法用于进行带有负采样的图块合并。
    首先，根据输入的边 ID 构建边子图。
    然后，对于每条边，通过负采样器获取负采样的源节点和目标节点。
    接着，根据邻居采样器从全图中采样邻居节点。
    最后，将采样的边子图和负采样子图进行压缩，并返回结果。
    
    3.collator 方法：
    
    此方法是 EdgeCollator 类的接口方法，用于对输入的边 ID 进行合并。
    在合并完成后，从合并后的子图中移除额外存储的属性，以减少内存占用。
...
'''
    ...
    接受一系列参数，包括输入图 g、边 ID eids、邻居采样器 graph_sampler 等。
    还可以接受其他关键字参数，以供传递给 EdgeCollator 和 torch.utils.data.DataLoader。
    
    2.__iter__ 方法：
    
    返回一个迭代器，用于迭代生成图数据加载器中的每个批次。
'''
