...
class GetEdgeEmbed(nn.Module):
    def __init__(self, emb_dim):
        super(GetEdgeEmbed, self).__init__()
        self.src_fc = nn.Linear(emb_dim, emb_dim)
        self.dst_fc = nn.Linear(emb_dim, emb_dim)
        self.out_fc = nn.Linear(emb_dim * 2, 1)

    def edge_embed(self, edges):
        src_hid = self.src_fc(edges.src['embedding'])
        dst_hid = self.dst_fc(edges.dst['embedding'])
        embedding = torch.cat((edges.src['embedding'], edges.dst['embedding']), dim=1)
    ...
    def forward(self, x, g):
        # Local Scope
        g.ndata['embedding'] = x
        g.apply_edges(self.edge_embed)
        edge_embed = g.edata['embedding']
        return edge_embed
...
class GetEdgeFeature(nn.Module):
    def __init__(self, emb_dim):
        ...
        self.src_fc = nn.Linear(emb_dim, emb_dim)
        self.dst_fc = nn.Linear(emb_dim, emb_dim)
        self.stats_fc = nn.Linear(12, emb_dim)
        self.out_fc = nn.Linear(emb_dim, 128)

    def edge_feat(self, edges):
        # src_hid = self.src_fc(edges.src['embedding'])
        # dst_hid = self.dst_fc(edges.dst['embedding'])
        # stats_hid = self.stats_fc(edges.data['stats_feats'])
        edge_feature = torch.cat((edges.src['embedding'], edges.dst['embedding'], edges.data['stats_feats']), dim=1)

        # edge_feature = F.relu(src_hid + dst_hid + stats_hid)
        # edge_feature = self.out_fc(edge_feature)
        return {'edge_feature': edge_feature}

    def forward(self, g):
        g.apply_edges(self.edge_feat)
        edge_feature = g.edata['edge_feature']
        return edge_feature
...
class TimeEncode(nn.Module):
    ...
    def __init__(self, dimension):
        ...
        self.w.weight = torch.nn.Parameter((torch.from_numpy(1 / 10 ** np.linspace(0, 9, dimension))).double().reshape(dimension, -1))
...
class MemoryOperation(nn.Module):
    ...
    def __init__(self, updater_type, memory, e_feat_dim, temporal_encoder):
        ...
        self.updater = updater_dict[updater_type](input_size=self.message_dim, hidden_size=memory_dim)
        self.memory = memory
...
class TemporalEdgePreprocess(nn.Module):
    ...
    def edge_fn(self, edges):
        ...
        time_encode = self.temporal_encoder(time_diff.unsqueeze(dim=1)).view(t0.shape[0], -1)
